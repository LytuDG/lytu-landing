...
export default function AICommandCenter() {
  ...
  useEffect(() => {
    ...
    // Ensure translation exists and is not just the key
    if (!nudgeText || nudgeText === "aiNavigator.botMessages.nudge") {
      console.warn("Nudge translation missing or empty");
      return;
    }

    const hasBeenNudged = false; // Allow showing once per page load for now
    if (!hasBeenNudged) {
  ...
  }, [isChatbotPage, t]); // dependencies are stable now
  ...
  const handleSearch = async () => {
    ...
    try {
      const rawIntent = await AIService.getIntent(input, currentLang);
      const [intentRaw, targetRaw] = rawIntent.split(":");
      const intent = intentRaw.trim().toLowerCase();
      const target = targetRaw?.trim().toLowerCase();

      // Internationalized messages
      let displayMessage = t("aiNavigator.botMessages.serviceOption", {
        intent,
      });
      if (target) {
        displayMessage = t("aiNavigator.botMessages.serviceTarget", { target });
      }

      if (target === "ai" || target === "design") {
        displayMessage = t("aiNavigator.botMessages.serviceSpecial", {
          target,
        });
      }
      ...
      const event = new CustomEvent("lytu-ai-intent", {
        detail: { intent, target, message: displayMessage },
      });
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3 pointer-events-none">
        ...
        {showBotMessage && (
          <div className="pointer-events-auto animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div className="bg-linear-to-br from-slate-900 via-slate-900 to-indigo-950 backdrop-blur-2xl border border-white/10 p-4 rounded-2xl shadow-2xl shadow-cyan-500/10 max-w-[220px] relative ring-1 ring-white/5">
              <button
              ...
              <div className="flex gap-3">
                <div className="shrink-0 w-9 h-9 rounded-xl bg-linear-to-br from-cyan-500 to-indigo-600 flex items-center justify-center text-white shadow-lg shadow-cyan-500/20">
                  <Bot size={18} />
                </div>
                <div>
                  <p className="text-[12px] text-white leading-snug font-bold">
                    {t("aiNavigator.botMessages.nudge")}
                  </p>
                  <Link
                    to="/demos/ai-chatbot"
                    className="inline-flex items-center gap-1 mt-2 text-[10px] font-black text-cyan-400 uppercase tracking-widest hover:text-cyan-300 transition-colors group/link"
                  >
                    {t("aiNavigator.botMessages.talkNow")}
                    <ArrowRight
                      size={10}
              ...
              {/* Tail */}
              <div className="absolute right-6 -bottom-1.5 w-3 h-3 bg-indigo-950 border-r border-b border-white/10 rotate-45" />
            </div>
          </div>
        )}
